...
export default function PlaySessionPage() {
    ...
    return (
      <div className="fixed inset-0 bg-slate-950 z-[100] flex flex-col items-center justify-center gap-8">
        <div className="relative">
          <div className="w-24 h-24 border-4 border-pink-900 rounded-3xl rotate-45 animate-pulse"></div>
        ...
        <div className="text-center space-y-2">
          <p className="text-white font-black tracking-[0.2em] uppercase text-sm">Synchronizing Arena</p>
          <p className="text-slate-500 text-xs font-bold italic animate-pulse">"Nurani is preparing your challenge..."</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="max-w-xl w-full animate-in zoom-in duration-500">
          <div className="bg-slate-900 rounded-[3rem] border border-slate-800 shadow-2xl overflow-hidden text-center">
             <div className="bg-slate-950 p-12 text-white space-y-4">
             ...
             <div className="p-12 space-y-10">
                <div className="flex flex-col items-center gap-2">
                   <p className="text-[10px] uppercase font-black text-slate-500 tracking-[0.3em]">Total Score</p>
                   <p className="text-6xl font-black text-white drop-shadow-sm">{totalScore.toLocaleString()}</p>
                   <div className="flex items-center gap-4 mt-2">
                      <div className="px-3 py-1 bg-green-500/10 text-green-400 rounded-full text-xs font-bold border border-green-500/20">
                        {correctCount} Benar
                      </div>
                      <div className="px-3 py-1 bg-red-500/10 text-red-400 rounded-full text-xs font-bold border border-red-500/20">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                   <button onClick={() => window.location.reload()} className="flex-1 flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-white py-4 rounded-2xl font-bold transition-all border-2 border-slate-700">
                     <RefreshCcw size={18} />
                     <span>REPLAY</span>
                   </button>
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center relative overflow-hidden text-white">
       ...
       {showFeedback && (
         <div className={`fixed inset-0 z-[200] flex flex-col items-center justify-center animate-in zoom-in fade-in duration-300 ${
           ...
           <div className="bg-slate-900 p-10 rounded-[3rem] shadow-2xl flex flex-col items-center gap-6 transform animate-bounce">
              ...
              <div className="text-center">
                 ...
                 {showFeedback === 'correct' && (
                    <p className="text-green-400 font-black text-xl mt-2 animate-pulse">
       ...
       <div className="w-full max-w-3xl flex-1 flex flex-col p-4 md:p-8 relative z-10">
          ...
          {/* Header Bar */}
          <div className="flex items-center justify-between bg-slate-900 px-6 py-4 rounded-3xl border border-slate-800 shadow-lg mb-6 shrink-0 relative overflow-hidden">
             ...
             <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-slate-800 border border-slate-700 text-white flex items-center justify-center font-black text-sm shadow-md">
                   {currentIndex + 1}
                </div>
                <div className="flex flex-col">
                   <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Cumulative Score</p>
                   <p className="font-black text-pink-500 text-lg leading-none">{totalScore.toLocaleString()}</p>
                </div>
             </div>

             <div className={`flex items-center gap-3 px-5 py-2 rounded-2xl font-black ${timeLeft <= 5 ? 'bg-red-900/30 text-red-400 border border-red-500/30 animate-pulse' : 'bg-slate-800 text-slate-300 border border-slate-700'}`}>
                <Clock size={18} />
          ...
          {/* Arena Soal */}
          <div className="flex-1 overflow-y-auto no-scrollbar rounded-[2.5rem] bg-slate-900 border border-slate-800 shadow-2xl flex flex-col">
                      ...
                      return (
                        <button
                          ...
                          className={`group flex items-center gap-4 p-4 rounded-[1.2rem] transition-all duration-300 text-left font-bold border-2 ${
                            isSelected 
                              ? 'bg-slate-800 border-pink-500 text-white scale-[0.98] shadow-[0_0_15px_rgba(236,72,153,0.3)]' 
                              : 'bg-slate-950 border-slate-800 hover:border-pink-500 hover:bg-slate-800 text-slate-300'
                          }`}
                        >
                          <div className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center font-black text-[10px] transition-all ${
                            isSelected ? 'bg-pink-500 text-white' : 'bg-slate-800 text-slate-500 border border-slate-700 group-hover:bg-pink-500/20 group-hover:text-pink-400 group-hover:border-pink-500/30'
                          }`}>
