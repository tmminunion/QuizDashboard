...
export default function PlaySessionPretestPage() {
  const { id } = useParams();
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-white z-[100] flex flex-col items-center justify-center gap-6">
        <div className="w-20 h-20 border-8 border-indigo-100 border-t-indigo-500 rounded-full animate-spin"></div>
        <p className="text-slate-400 font-black tracking-[0.3em] uppercase animate-pulse">Entering Pretest Arena...</p>
      </div>
    );
  }
        ...
        <div className="max-w-xl w-full animate-in zoom-in duration-500">
          <div className="bg-white rounded-[3rem] border border-slate-800 shadow-2xl overflow-hidden text-center">
             <div className="bg-slate-900 p-12 text-white space-y-4">
                <div className="inline-flex p-6 bg-indigo-500 rounded-full shadow-[0_0_40px_rgba(99,102,241,0.5)] mb-2">
                   <Trophy size={48} className="fill-white" />
                </div>
                <h2 className="text-3xl font-black uppercase tracking-tight">Pretest Selesai!</h2>
                <p className="text-indigo-400 font-bold text-sm italic">"Pangeran Nunu luar biasa!"</p>
             </div>
             ...
             <div className="p-12 space-y-10">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                   ...
                   <Link href="/" className="flex-1 flex items-center justify-center gap-2 bg-indigo-500 hover:bg-indigo-600 text-white py-4 rounded-2xl font-bold shadow-lg">
                     <Home size={18} />
                     <span>EXIT</span>
                   </Link>
                </div>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center relative overflow-hidden">
       ...
       <div className="w-full max-w-3xl flex-1 flex flex-col p-4 md:p-8 relative z-10">
          ...
          <div className="flex items-center justify-between bg-white px-6 py-4 rounded-3xl border border-slate-200 shadow-lg mb-6 shrink-0 relative overflow-hidden">
             <div className="absolute top-0 left-0 h-1 bg-indigo-500 transition-all duration-700" style={{ width: `${((currentIndex + 1) / questions.length) * 100}%` }}></div>
             ...
             <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                   ...
                   <p className="font-black text-indigo-600 text-lg leading-none">{totalScore.toLocaleString()}</p>
                </div>
             </div>
             ...
             <div className="p-8 md:p-12 space-y-10 flex-1 flex flex-col justify-center">
                <div className="text-center space-y-2">
                   <div className="inline-flex items-center gap-2 text-indigo-500 font-black text-[10px] uppercase tracking-[0.3em]">
                      <Zap size={12} className="fill-indigo-500" /> Challenge Active
                   </div>
                        ...
                        <button
                          ...
                          className={`group flex items-center gap-4 p-4 rounded-[1.2rem] transition-all duration-300 text-left font-bold border-2 ${
                            isSelected 
                              ...
                              : 'bg-white border-slate-100 hover:border-indigo-500 hover:bg-indigo-50/5 text-slate-700'
                          }`}
                        >
