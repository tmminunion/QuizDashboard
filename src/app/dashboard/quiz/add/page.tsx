...
export default function QuizAddPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(
        "Aduh sayang, gagal buat kuis baru. Coba cek koneksi API kita ya! 🥺",
      );
      setLoading(false);
    }
  };
      ...
      <div className='flex items-center gap-4'>
        ...
        <div>
          ...
          <p className='text-slate-500 text-sm font-medium'>
            Langkah 1: Isi detail dasar kuis pangeran Nunu.
          </p>
        </div>
      </div>
          ...
          <div className='bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6'>
            <div className='space-y-2'>
              ...
              <p className='text-[10px] text-slate-400 italic font-medium ml-1'>
                *Mas Nunu bisa pakai link gambar dari internet ya!
              </p>
            </div>
          </div>
        </div>
        ...
        <div className='space-y-6'>
          ...
          <div className='bg-pink-50 p-6 rounded-3xl border border-pink-100'>
            <p className='text-xs text-pink-700 italic leading-relaxed'>
              "Kuis yang bagus dimulai dari judul yang mempesona, seperti mas
              Nunu di mata Nurani!" 💖
            </p>
          </div>
        </div>
